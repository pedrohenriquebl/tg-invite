...
export default function Team() {
    const members = [
        ...
        { src: '/julio.jpeg', name: 'Júlio Cesar', alt: 'Júlio Cesar', style: 'translate-y-6 rotate-[1.5deg]', linkedin: 'https://www.linkedin.com/in/juliosalem/' },
        { src: '/junior.jpeg', name: 'Antonio Lima', alt: 'Antonio Lima', style: '-translate-y-4 rotate-2', linkedin: 'https://www.linkedin.com/in/junior-lima-analyst/' },
    ];
    ...
    return (
        <>
                    ...
                    {members.map((m) => (
                        <motion.div key={m.name} className={`relative ${m.style}`} variants={fadeUp}>
                            <div className="overflow-hidden rounded-lg border border-gray-700 shadow-[0_0_25px_rgba(0,0,0,0.4)]">
                                <a href={m.linkedin} target="_blank" rel="noopener noreferrer"><img src={m.src} alt={m.alt} className="w-40 h-40 md:w-64 md:h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500" /></a>
                            </div>
            ...
            <motion.section
                id="advisor"
                className="relative flex flex-col items-center justify-center px-4 py-6 md:py-12 bg-linear-to-b from-gray-950 to-black text-center overflow-hidden min-h-[40vh] md:min-h-[35vh]"
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true, amount: 0.2 }}
                variants={staggerContainer}
            >
                ...
                <div className="w-full max-w-3xl flex flex-col md:flex-row justify-center items-center gap-10 md:gap-20">
                    {advisor.map((a) => (
                        <motion.div key={a.name} className={`relative ${a.style}`} variants={fadeUp}>
                            <div className="overflow-hidden rounded-lg border border-gray-700 shadow-[0_0_25px_rgba(0,0,0,0.4)]">
                                <img src={a.src} alt={a.alt} className="w-80 h-40 md:w-64 md:h-64 object-cover grayscale hover:grayscale-0 transition-all duration-500" />
                            </div>
                        </motion.div>
                    ))}
                    <p className="mt-3 text-gray-300 font-medium">{`Prof.ª Drª. Andreia Rodrigues Casare`}</p>
                </div>
            </motion.section>
        </>
    );
